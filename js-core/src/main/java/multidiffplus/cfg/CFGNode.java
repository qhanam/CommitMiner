...
public class CFGNode {
	 ...
	 * @param node The node at the other end of this edge.
	 */
	public void addOutgoingEdge(ClassifiedASTNode condition, CFGNode node, boolean loopEdge, int id) {
		CFGEdge edge = this.getOutgoingEdge(condition);
	...
	public List<CFGNode> getAdjacentNodes() {
		...
		for(CFGEdge edge : this.outEdges) nodes.add(edge.getTo());
		return nodes;
	}
	...
	public static CFGNode copy(CFGNode node) {
        ...
        for(CFGEdge edge : node.getOutgoingEdges()) newNode.addOutgoingEdge(edge.getCondition(), edge.getTo(), edge.getId());
        return newNode;
	}
