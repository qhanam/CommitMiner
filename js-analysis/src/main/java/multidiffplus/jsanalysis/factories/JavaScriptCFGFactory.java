...
public class JavaScriptCFGFactory implements ICFGFactory {
	...
	ifNode.addOutgoingEdge(new CFGEdge(falseBranchCondition, ifNode, falseBranch.getEntryNode(),
		idgen.getUniqueID()));
	...
	for (SwitchCase switchCase : switchCases) {
		...
		switchNode.addOutgoingEdge(new CFGEdge(compare, switchNode, subGraph.getEntryNode(),
			idgen.getUniqueID()));
	...
	if (defaultEdge == null) {
	    ...
	    defaultEdge = new CFGEdge(null, switchNode,
		    new CFGNode(new EmptyStatement(), idgen.getUniqueID()), idgen.getUniqueID());
	...
	for (CatchClause catchClause : catchClauses) {
		 ...
		 * CFG.
		 */
		cfg.addAllBreakNodes(moveJumpAfterFinally(finallyBlock, catchBlock.getBreakNodes(),
			catchCondition, idgen));
		...
		cfg.addAllThrowNodes(moveJumpAfterFinally(finallyBlock, catchBlock.getThrowNodes(),
			catchCondition, idgen));
     ...
     */
    private static List<CFGNode> moveJumpAfterFinally(CFG finallyBlock, List<CFGNode> jumpNodes,
