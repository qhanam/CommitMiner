...
public class JavaScriptCFGFactory implements ICFGFactory {
			...
			if(subGraph != null) {
				...
				if(previous == null) {
					...
					for(CFGNode exitNode : previous.getExitNodes()) {
						exitNode.addOutgoingEdge(null, subGraph.getEntryNode(), idgen.getUniqueID());
					}
				}
	...
	private static CFG build(IfStatement ifStatement, IDGen idgen) {
		...
		ifNode.addOutgoingEdge(new CFGEdge(ifStatement.getCondition(), ifNode, trueBranch.getEntryNode(), idgen.getUniqueID()));
		...
		ifNode.addOutgoingEdge(new CFGEdge(falseBranchCondition, ifNode, falseBranch.getEntryNode(), idgen.getUniqueID()));
	...
	private static CFG build(WhileLoop whileLoop, IDGen idgen) {
		...
		whileNode.addOutgoingEdge(new CFGEdge(whileLoop.getCondition(), whileNode, trueBranch.getEntryNode(), true, idgen.getUniqueID()));
        ...
        CFGNode empty = new CFGNode(new EmptyStatement(), idgen.getUniqueID());
		whileNode.addOutgoingEdge(new CFGEdge(falseBranchCondition, whileNode, empty, idgen.getUniqueID()));
	...
	private static CFG build(ForLoop forLoop, IDGen idgen) {
		...
		condition.addOutgoingEdge(forLoop.getCondition(), trueBranch.getEntryNode(), true, idgen.getUniqueID());
	...
	private static CFG build(ForInLoop forInLoop, IDGen idgen) {
        ...
        condition.addOutgoingEdge(new CFGEdge(keyConditionFunction, condition, assignment, true, idgen.getUniqueID()));
		...
		condition.addOutgoingEdge(new CFGEdge(falseBranchCondition, condition, falseBranch, idgen.getUniqueID()));
	...
	private static CFG build(SwitchStatement switchStatement, IDGen idgen) {
		...
		for(SwitchCase switchCase : switchCases) {
            ...
            if(switchCase.getExpression() != null) {
                ...
                switchNode.addOutgoingEdge(new CFGEdge(compare, switchNode, subGraph.getEntryNode(), idgen.getUniqueID()));
	...
	private static CFG build(TryStatement tryStatement, IDGen idgen) {
		...
		for(CatchClause catchClause : catchClauses) {
			...
			tryNode.addOutgoingEdge(catchCondition, catchBlock.getEntryNode(), idgen.getUniqueID());
		...
		if(tryBlock == null) {
            ...
            for(CFGNode throwNode : tryBlock.getThrowNodes()) {
            	throwNode.addOutgoingEdge(null, finallyBlock.getEntryNode(), idgen.getUniqueID());
            }
