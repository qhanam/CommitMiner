...
public class TransferNode {
	...
	public void interpretReturn(ReturnStatement rs) {
		...
		if(rs.getReturnValue() == null) {
			retVal = Undefined.inject(Undefined.top(), Change.convU(rs), DefinerIDs.inject(rs.getID()));
		}
		else {
		...
		state.env = state.env.strongUpdate("~retval~", new Variable(rs.getID(), "~retval~", new Addresses(address)));
