...
public class TransferEdge {
	...
	private void interpretAddrsFalsey(Set<Address> addrs) {
		...
		for(Address addr : addrs) {
			...
			BValue val = Undefined.inject(Undefined.top(), oldVal.change, oldVal.definerIDs)
					...
					.join(Str.inject(new Str(Str.LatticeElement.SBLANK), oldVal.change, DefinerIDs.bottom()))
					.join(Num.inject(new Num(Num.LatticeElement.NAN_ZERO), oldVal.change, DefinerIDs.bottom()))
					.join(Bool.inject(new Bool(Bool.LatticeElement.FALSE), oldVal.change, DefinerIDs.bottom()));
	...
	private void interpretEQ(InfixExpression ie) {
		...
		if(BValue.isBlank(rhsVal) || BValue.isZero(rhsVal))
			for(Address lhsAddr : lhsAddrs) {
				BValue oldVal = state.store.apply(lhsAddr);
				rhsVal = Str.inject(new Str(Str.LatticeElement.SBLANK), oldVal.change, oldVal.definerIDs)
						.join(Num.inject(new Num(Num.LatticeElement.ZERO), oldVal.change, oldVal.definerIDs));
				state.store.strongUpdate(lhsAddr, rhsVal);
			}
		if(BValue.isNaN(rhsVal))
			for(Address lhsAddr : lhsAddrs) {
				BValue oldVal = state.store.apply(lhsAddr);
				state.store.strongUpdate(lhsAddr, Num.inject(new Num(Num.LatticeElement.NAN), oldVal.change, oldVal.definerIDs));
			}
		if(BValue.isFalse(rhsVal))
