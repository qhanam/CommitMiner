...
public class Helpers {
	...
	public static Store createFunctionObj(Closure closure, Store store, Trace trace, Address address, FunctionNode function) {
		...
		store = addProp(function.getID(), "length", Num.inject(Num.top(), Change.u(), DefinerIDs.bottom()), external, store, trace);
	...
	public static Store lift(Environment env,
		...
		for(Name localVar : localVars) {
			...
			env.strongUpdateNoCopy(localVar.toSource(), new Variable(localVar.getID(), localVar.toSource(), Change.convU(localVar), new Addresses(address)));
			store = store.alloc(address, Undefined.inject(Undefined.top(), Change.u(), DefinerIDs.bottom()));
		}
		...
		for(FunctionNode child : children) {
			...
			env.strongUpdateNoCopy(child.getName(), new Variable(child.getID(), child.getName(), Change.convU(child.getFunctionName()), new Addresses(address))); // Env update with env change type
			store = store.alloc(address, Address.inject(address, Change.convU(child), DefinerIDs.inject(child.getID())));
