...
/**
 * Stores the state for the null type abstract domain. Lattice element is
 * simple: TOP | BOT Where TOP means the type could be null and BOT means the
 * type is definitely not null.
 */
public class Null {
     ...
     * 
     * @param le
     *            The lattice element to merge with.
     * @return The updated state for this domain.
     */
    public Null weakUpdate(LatticeElement le) {
	if (this.le == le)
	    return new Null(this.le);
    ...
    /**
     * @param nll
     *            The null lattice element to inject.
     * @return The base value tuple with injected null.
     */
    public static BValue inject(Null nll, Change valChange, Dependencies dependencies) {
	return BValue.inject(Str.bottom(), Num.bottom(), Bool.bottom(), nll, Undefined.bottom(),
		Addresses.bottom(), valChange, dependencies);
    }
