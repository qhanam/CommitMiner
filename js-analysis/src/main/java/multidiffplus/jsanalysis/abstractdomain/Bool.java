...
/**
 * Stores the state for the boolean type abstract domain. Lattice element is
 * simple: TOP / \ true false \ / BOT Where TOP means the type could be a
 * boolean and BOT means the type is definitely not a boolean.
 */
public class Bool {
    ...
    /**
     * @param bool
     *            The boolean lattice element to inject.
     * @return The base value tuple with injected boolean.
     */
    public static BValue inject(Bool bool, Change valChange, Dependencies deps) {
	return BValue.inject(Str.bottom(), Num.bottom(), bool, Null.bottom(), Undefined.bottom(),
