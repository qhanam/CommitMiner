...
/**
 * Stores the state for the undefined type abstract domain. Lattice element is
 * simple: TOP | BOT Where TOP means the type could be undefined and BOT means
 * the type is definitely not undefined.
 */
public class Undefined {
     ...
     * 
     * @param state
     *            The undefined to join with.
     * @return A new undefined that is the join of the two addresses.
     */
    public Undefined join(Undefined state) {
	if (this.le == state.le)
	    return new Undefined(this.le);
	return new Undefined(LatticeElement.TOP);
    }

    /**
     * @param undefined
     *            The undefined lattice element to inject.
     * @return The base value tuple with injected undefined.
     */
    public static BValue inject(Undefined undefined, Change valChange, Dependencies deps) {
	return BValue.inject(Str.bottom(), Num.bottom(), Bool.bottom(), Null.bottom(), undefined,
