...
public class Addresses {
    ...
    /**
     * @param addresses
     *            The address lattice element to inject.
     * @return The base value tuple with injected addresses.
     */
    public static BValue inject(Addresses addresses, Change valChange, Dependencies deps) {
	return BValue.inject(Str.bottom(), Num.bottom(), Bool.bottom(), Null.bottom(),
		Undefined.bottom(), addresses, valChange, deps);
    }

    public static BValue dummy(Change valChange, Dependencies deps) {
	return BValue.inject(Str.top(), Num.top(), Bool.top(), Null.top(), Undefined.top(),
	...
	if (this.addresses.size() != addrs.addresses.size())
	    return false;
	for (Address addr : this.addresses) {
	    if (!addrs.addresses.contains(addr))
		return false;
	}
	return true;
    }
