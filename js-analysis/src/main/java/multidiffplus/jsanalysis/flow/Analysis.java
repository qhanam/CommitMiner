...
public class Analysis {
	...
	if (!stackFrame.isFinished()) {
	    ...
	    Instruction instruction = stackFrame.instrStack.pop();

	    switch (stackFrame.progress) {
	    case NONE:
		...
		if (postTransferState == null) {
		    ...
		    timer.suspend();
		    advance(); // Problem here. Why is postTransferState null?
		}
	    case EDGE:
	    default:
	...
	/*
	 * We have reached the end of the function, so analyze the publicly accessible
	...
	List<Name> localVarNames = VariableLiftVisitor
		.getVariableDeclarations((ScriptNode) stackFrame.cfg.getEntryNode().getStatement());
	...
	List<FunctionNode> localFunctions = FunctionLiftVisitor
		.getFunctionDeclarations((ScriptNode) stackFrame.cfg.getEntryNode().getStatement());
	...
	Helpers.analyzeEnvReachable(exitState, exitState.env.environment, exitState.selfAddr, cfgs,
     ...
     * 
     * @return The abstract state after the transfer, or null if a function call was
     *         added to the stack.
     */
    private State transferNode(StackFrame stackFrame, Instruction instruction) {
	...
	/*
	 * If it does not exist, put it in the map and initialize the semaphore value to
	...
	transferFunction = new TransferNode(postTransferState, instruction.edge.getTo(),
		evaluator(postTransferState));
	transferFunction.transfer();

	/*
	 * If the transfer function requested a new call be pushed onto the stack, we
	 * need to (1) 'pause' this analysis (by pushing this instruction back onto the
	...
	for (CFGEdge edge : instruction.edge.getTo().getOutgoingEdges()) {
	    ...
	    /*
	     * Only visit an edge if the semaphore for the current node is zero or if one of
		...
		stackFrame.instrStack.push(newState);
	    }
     ...
     *         added to the stack.
     */
    private State transferEdge(StackFrame stackFrame, Instruction pathState) {

	/*
	 * Join the lattice elements from the current edge and 'from' node.
	 */
	State preTransferState;
	preTransferState = ((State) pathState.edge.getFrom().getAfterState())
		.join((State) pathState.edge.getBeforeState());
	...
	transferFunction = new TransferEdge(postTransferState, pathState.edge,
		evaluator(postTransferState));
	transferFunction.transfer();
	 ...
	 */
	if (this.frameToLoad != null) {
	    stackFrame.instrStack.push(pathState);
	    this.callStack.push(stackFrame);
	    this.callStack.push(this.frameToLoad);
	    this.frameToLoad = null;
	    return null;
	...
	pathState.edge
		.setAfterState(postTransferState.join((State) pathState.edge.getAfterState()));

	/*
	 * Look up the number of times this node has been visited in the
	 * visitedSemaphores map.
	 */
	Integer semVal = stackFrame.iesMap.get(pathState.edge.getTo());
	 ...
	 */
	if (semVal == null)
	    semVal = pathState.edge.getTo().getIncommingEdgeCount();

	/* Decrement the semaphore by one since we visited the node. */
	semVal = semVal - 1;
	stackFrame.iesMap.put(pathState.edge.getTo(), semVal);
     ...
     * unvisited loop edge. If semVal == -1, do not proceed because a call has been
