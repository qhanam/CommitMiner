...
public class HTMLMultiDiffViewer {
     ...
     * 
     * @param inputPath
     *            The original source code file (text)
     * @param alerts
     *            The alerts used to annotate the file
     * @return The annotated file
     * @throws IOException
     *             when files cannot be read or written
     */
    public static String annotate(String source, AnnotationFactBase factBase) throws IOException {
	...
	for (int i = 0; i < chars.length; i++) {
		...
		while (it.hasNext()) {
		    ...
		    out = out.concat("<span class='context-menu " + openTag.label
			    + "' data-address='" + openTag.getDependencyLabel() + "'>");
		}
	    }
		...
		/* Open the tag. */
		out = out.concat("<span class='context-menu " + current.label + "' data-address='"
			+ current.getDependencyLabel() + "'>");
