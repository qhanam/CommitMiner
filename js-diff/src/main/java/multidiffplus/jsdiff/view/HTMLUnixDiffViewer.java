...
public class HTMLUnixDiffViewer {
     ...
     * @param dstCode
     *            The original text of the destination file
     * @param dstAnnotated
     *            The annotated text of the destination file
     * @throws IOException
     *             when files cannot be read or written
     */
    public static String annotate(String srcCode, String dstCode, String dstAnnotated)
	    throws IOException {
	...
	for (DiffMatchPatch.Diff diff : diffs) {
	    ...
	    /*
	     * If this is a DELETE operation, we track the number of lines removed so we can
	     * align the deleted lines with the inserted lines.
	     */
	    if (diff.operation == DiffMatchPatch.Operation.DELETE) {
		if (sem > 0)
		    throw new Error("Invalid state: DELETE operation with sem > 0");
		sem = diff.text.length();
	    }

	    /*
	     * If an EQUALS diff follows a DELETE diff, we need to add blank lines in the
	     * destination file to align the diff.
	     */
		...
		case EQUAL:
			...
			outSrc += "<td class='line'>" + i + "</td><td>"
				+ srcLines[i - 1].replace("\t", "  ") + "</td>\n";
		    else
			outSrc += "<td class='line'>" + i + "</td><td></td>\n";
		    if (dstLines.length >= j)
			outDst += "<td class='line'>" + j + "</td><td>"
				+ dstLines[j - 1].replace("\t", "  ") + "</td>\n";
		    else
		...
		case DELETE:
		    ...
		    i++;
		    if (srcLines.length >= i)
			outSrc += "<td class='line deleteLine'>" + i + "</td><td class='delete'>"
				+ srcLines[i - 1].replace("\t", "  ") + "</td>\n";
		    else
			outSrc += "<td class='line deleteLine'>" + i
				+ "</td><td class='delete'></td>\n";
		    break;
		case INSERT:
		    // Print destination line and, if needed, blank lines in source
		    j++;
		    if (dstLines.length >= j)
			outDst += "<td class='line insertLine'>" + j + "</td><td class='insert'>"
				+ dstLines[j - 1].replace("\t", "  ") + "</td>\n";
		    else
			outDst += "<td class='line insertLine'>" + j
				+ "</td><td class='insert'></td>\n";

		    /* Add lines to the source file as needed. */
		    if (sem > 0)
			sem--;
		    else
			outSrc += "<td class='line alignment'></td><td class='alignment'></td>\n";
	...
	if (srcRows.length != dstRows.length)
	    throw new Error("SRC rows and DST rows should be equal");
