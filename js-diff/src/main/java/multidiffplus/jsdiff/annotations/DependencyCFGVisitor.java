 ...
 */
public class DependencyCFGVisitor implements ICFGVisitor {

    /* The fact database we will populate. */
    private AnnotationFactBase factBase;

    public DependencyCFGVisitor(AnnotationFactBase factBase) {
	...
	if (state != null) {
	    Set<Annotation> annotations = ValueASTVisitor.getAnnotations(node);
