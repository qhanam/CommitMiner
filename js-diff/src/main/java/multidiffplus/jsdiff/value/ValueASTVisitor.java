...
public class ValueASTVisitor implements NodeVisitor {
	...
	if (statement instanceof AstRoot) {
		...
		visitor.annotations.add(new Annotation("VAL-DEF", ids, function.getLineno(),
	...
	if (node instanceof Name) {
	    ...
	    if (ids.size() > 0) {
		this.annotations.add(new Annotation("VAL-USE", ids, pg.getRight().getLineno(),
	    ...
	    if (ids.size() > 0) {
		this.annotations.add(new Annotation("VAL-USE", ids, pg.getElement().getLineno(),
			pg.getElement().getFixedPosition(), pg.getElement().getLength()));
	    }
		...
		List<DependencyIdentifier> ids = new LinkedList<DependencyIdentifier>();
		ids.add(val);
		this.annotations.add(new Annotation("VAL-USE", ids, prop.getLineno(),
	...
	/* Register VAL-DEF annotations for literals. */
		...
		case Token.FALSE:
		    ...
		    this.annotations.add(new Annotation("VAL-DEF", ids, kwl.getLineno(),
		...
		this.annotations.add(new Annotation("VAL-DEF", ids, node.getLineno(),
			node.getFixedPosition(), node.getLength()));
	    }
