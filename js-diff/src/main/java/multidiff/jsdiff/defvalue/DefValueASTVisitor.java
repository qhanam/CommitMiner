...
public class DefValueASTVisitor implements NodeVisitor {
	...
	if (statement instanceof AstRoot) {
	    ...
	    visitor.annotations.add(new Annotation("DVAL-DEF", ids, function.getLineno(),
		    function.getFixedPosition(), 8));
	    function.addCriterion("VALUE", function.getID());
	...
	if (node.isDummy()) {
	    ...
	    this.annotations.add(new Annotation("DVAL-DEF", ids, node.getLineno(),
		    node.getFixedPosition(), node.getLength()));
	    node.addCriterion("VALUE", node.getID());
	...
	if (node instanceof Name) {
	    ...
	    if (ids.size() > 0) {
		this.annotations.add(new Annotation("DVAL-USE", ids, pg.getLineno(),
			pg.getFixedPosition(), pg.getLength()));
		for (DependencyIdentifier identifier : ids) {
		    for (Integer id : identifier.getAddresses()) {
			node.addDependency("VALUE", id);
		    }
	    ...
	    /*
	     * Get the object from the store, using the address re-constructed from Trace.
	     */
	    ...
	    List<DependencyIdentifier> ids = new LinkedList<DependencyIdentifier>();
	    ids.add(val);
	    this.annotations.add(new Annotation("DVAL-USE", ids, prop.getLineno(),
		    prop.getFixedPosition(), prop.getLength()));
	    for (Integer id : val.getAddresses()) {
		prop.addDependency("VALUE", id);
	    }
	...
	/* Register DVAL-DEF annotations for literals. */
		...
		this.annotations.add(new Annotation("DVAL-DEF", ids, kwl.getLineno(),
			kwl.getFixedPosition(), kwl.getLength()));
		kwl.addCriterion("VALUE", kwl.getID());
	    }
	    ...
	    this.annotations.add(new Annotation("DVAL-DEF", ids, node.getLineno(),
		    node.getFixedPosition(), node.getLength()));
	    node.addCriterion("VALUE", node.getID());
	} else if (node instanceof ObjectLiteral || node instanceof ArrayLiteral) {
	    List<DependencyIdentifier> ids = new LinkedList<DependencyIdentifier>();
	    ids.add(new GenericDependencyIdentifier(node.getID()));
	    this.annotations.add(
		    new Annotation("DVAL-DEF", ids, node.getLineno(), node.getFixedPosition(), 1));
	    this.annotations.add(new Annotation("DVAL-DEF", ids, node.getLineno(),
		    node.getFixedPosition() + node.getLength() - 1, 1));
	    node.addCriterion("VALUE", node.getID());
	}
