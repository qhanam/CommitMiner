...
CLI.getVersion = function(cb) {
  Satan.executeRemote('getVersion', {}, function(err, data) {
...
/**
 * Compute the logging line buffer.
 */
function getLineBuffer() {
	above = Log.getLeadingLines();	
	below = Log.getTailingLines();
	below =  below + 2;
	a = 5;
	buffer = 2 * above + 2 * below;
	return buffer;
}

buffer = getLineBuffer();
CLI.streamLogs("1", buffer, "content", new Date(), true);
